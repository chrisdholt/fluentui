...
import { FluentAnchoredRegion } from './index';
import './index';
  ...
  if (name.toLowerCase().startsWith('anchored-region')) {
    ...
    document.querySelectorAll('[id^=toggle-anchor-anchor').forEach(anchor => {
      anchor.addEventListener('click', (e: MouseEvent) => {
        document.getElementById('toggle-anchor-region')!.setAttribute('anchor', (e.target as HTMLElement).id);
      });
    });

    const positionsRegion = document.getElementById('toggle-positions-region')!;
    document.querySelectorAll('#toggle-positions-horizontal, #toggle-positions-vertical').forEach((el: HTMLElement) => {
      el.addEventListener('click', (e: MouseEvent) => {
        const isHorizontal = (e.target as HTMLElement).id.includes('horizontal');
        ...
        positionsRegion.setAttribute(
          attr,
          isHorizontal ? (currentPosition === 'left' ? 'right' : 'left') : currentPosition === 'top' ? 'bottom' : 'top',
        );
      });
    });
    ...
    document.querySelectorAll('[id^=btn-toggle-positions]').forEach((el: HTMLElement) => {
      el.addEventListener('click', (e: MouseEvent) => {
    ...
    const regionScalingUpdate = document.getElementById('region-scaling-update') as FluentAnchoredRegion;
    document.getElementById('viewport-scaling-update')!.addEventListener('scroll', () => regionScalingUpdate.update());

    const regionScalingOffset = document.getElementById('region-scaling-offset') as FluentAnchoredRegion;
    document.getElementById('viewport-scaling-offset')?.addEventListener('scroll', (e: MouseEvent) => {
